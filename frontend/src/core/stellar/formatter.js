...
export const STROOPS_UNIT = 10_000_000;
 ...
 */
export function convertXlmToStroops(amountXlm) {
  return BigInt(Math.floor(Number(amountXlm) * STROOPS_UNIT));
}
 ...
 */
export function convertStroopsToXlm(amountStroops) {
  if (!amountStroops) return "0";
  return (Number(amountStroops) / STROOPS_UNIT).toString();
}
