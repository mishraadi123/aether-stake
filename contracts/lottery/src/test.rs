...
fn setup_test(
    ...
    Address,
    Address,
    LotteryContractClient<'_>,
    ticket::TicketContractClient<'_>,
    treasury::TreasuryContractClient<'_>,
) {
    env.mock_all_auths();
    ...
    let token_id = env.register(TicketContract, ());
    let token_client = ticket::TicketContractClient::new(env, &token_id);
    ...
    let lottery_id = env.register(LotteryContract, ());
    let lottery_client = LotteryContractClient::new(env, &lottery_id);
    ...
    token_client.init(&lottery_id);
    treasury_client.init(&treasury_admin, &lottery_id, &xlm_token_addr);
...
fn test_lottery_initialization() {
    ...
    let round_id = lottery_client.get_current_round_id();
...
fn test_open_round_success() {
    ...
    let round_id = lottery_client.open_round(&3600);
    assert_eq!(round_id, 1);

    let info = lottery_client.get_round(&round_id);
...
fn test_open_round_fails_if_active() {
    ...
    lottery_client.open_round(&3600);
    lottery_client.open_round(&3600);
}
...
fn test_buy_ticket_success() {
    ...
    lottery_client.buy_ticket(&buyer1, &round_id);
    ...
    // Check ticket count
    assert_eq!(token_client.balance(&buyer1, &round_id), 1);
    assert_eq!(lottery_client.get_tickets(&round_id, &buyer1), 1);
...
fn test_settle_round_success() {
    ...
    lottery_client.buy_ticket(&buyer1, &round_id);
    lottery_client.buy_ticket(&buyer1, &round_id);
    lottery_client.buy_ticket(&buyer2, &round_id);
    lottery_client.buy_ticket(&buyer2, &round_id);
    ...
    // Settle round
    let winner = lottery_client.settle_round(&round_id);
    ...
    assert_eq!(treasury_client.total_fees(), 25);
...
fn test_treasury_unauthorized_deposit() {
    ...
    treasury_client.init(&admin, &lottery, &token);

    treasury_client.deposit_fee(&1, &100);
}
...
fn test_settle_round_fails_before_close_time() {
    ...
    lottery_client.settle_round(&round_id);
}
